  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // If session exists, show onboarding questionnaire
        // If no session, user needs to confirm email first
        if (data.session) {
          // Email confirmation is disabled - show onboarding questionnaire
          setNewUserId(data.user.id)
          setShowOnboarding(true)
          return
        } else {
          // Email confirmation required - show message to check email
          setError('Please check your email to confirm your account. After confirmation, you can sign in to access your dashboard.')
          // Don't set success to true - show error message instead
        }
      }
    } catch (err: any) {
  ...
  if (showOnboarding && newUserId) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header />
        <OnboardingQuestionnaire userId={newUserId} onComplete={handleOnboardingComplete} />
      </div>
    )
  }
        ...
        <div className="flex items-center justify-center px-4 py-12 min-h-[calc(100vh-80px)]">
          <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <p className="text-sm text-text-secondary mb-6">
              Click the link in the email to confirm your account and sign in to access your dashboard.
            </p>
            <Link
              href="/auth/login"
      ...
      <div className="flex items-center justify-center px-4 py-12 min-h-[calc(100vh-80px)]">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            {planName && (
              <p className="text-sm text-accent mt-2 font-medium">Plan: {planName}</p>
            )}
          </div>
